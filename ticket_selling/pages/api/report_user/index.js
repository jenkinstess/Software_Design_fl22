...
async function getReportedTickets(user_id, callback){
  const resultFound = await sequelize.query("SELECT * FROM ticketsitedb.tickets WHERE userUserid = :user_id AND is_sold = 0",
  {
    replacements: {user_id: user_id},
...
async function takeTixOffMarket(user_id){
  const resultFound = await sequelize.query("UPDATE ticketsitedb.tickets SET is_sold = 1 WHERE userUserid = :user_id",
  {
    replacements: {user_id: user_id},
    type: QueryTypes.UPDATE
  })
